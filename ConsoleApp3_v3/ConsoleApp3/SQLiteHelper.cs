...
namespace System.Data.SQLite
{
    ...
    /// Data Access Application Block v2.0 and follows similar pattern.
    /// </summary>
    public class SQLiteHelper
    {
        ...
        /// <param name="commandText">Command text.</param>
        /// <param name="paramArray">Parameter Values object array.</param>
        /// <returns></returns>
        public static DataSet ExecuteDataSet(SQLiteConnection cn, string commandText, object[] paramArray)
        {
            SQLiteCommand cmd = cn.CreateCommand();
            cmd.CommandText = commandText;
            if (paramArray != null)
            {
                cmd = DeriveParameters(cmd, paramArray);
            }
                                             ...
                                             params SQLiteParameter[] commandParameters)
        {
            ...
            SQLiteCommand cmd = transaction.Connection.CreateCommand();
            cmd.CommandText = commandText;
        ...
        public static DataSet ExecuteDataSet(string connectionSting, string commandText, object[] parameters)
        {
            SQLiteConnection cn = new SQLiteConnection(connectionSting);

            SQLiteCommand cmd = cn.CreateCommand();
            cmd.CommandText = commandText;
            if (parameters != null)
                cmd = DeriveParameters(cmd, parameters);
            cmd.Connection = cn;
            cn.Open();
            DataSet ds = ExecuteDataset((SQLiteCommand) cmd);
            return ds;
        ...
        /// <returns>DataSet - leaving transaction open</returns>
        /// <remarks>user must examine Transaction Object and handle transaction.connection .Close, etc.</remarks>
        public static DataSet ExecuteDataset(SQLiteTransaction transaction, string commandText,
                                             object[] commandParameters)
        {
            ...
            SQLiteCommand cmd = transaction.Connection.CreateCommand();
            cmd.CommandText = commandText;
            cmd = DeriveParameters(cmd, commandParameters);
        ...
        /// <param name="commandText">SQL Statement with optional embedded "@paramName" style parameters</param>
        ...
        public static XmlReader ExecuteXmlReader(SQLiteCommand command)
        {
        ...
        /// </summary>
        /// <param name="cmd">SQLite Command object</param>
        /// <param name="paramList">object[] array of parameter values</param>
        ...
        public static SQLiteCommand DeriveParameters(SQLiteCommand cmd, object[] paramList)
        {
            if (paramList == null) return cmd;
            string commandText = cmd.CommandText;
            if (commandText == null) throw new ArgumentException("Command text is empty", "cmd");
